<template>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
    ...
    >
      ...
      >
        <h2 class="text-lg font-bold text-white">{{ activity.name }}</h2>
        <div class="flex items-center space-x-2">
          <LucideCalendar class="w-5 h-5 text-shields-txt-secondary" />
          <p
            :class="{
              'text-orange-400': getDaysLeft(activity.date) === 0,
              'text-green-400': getDaysLeft(activity.date) !== 0,
            }"
          >
            {{
              getDaysLeft(activity.date) === 0
                ? new Date(activity.date).toLocaleDateString("fr-FR")
                : getDaysLeft(activity.date) === 1
                ? "dans 1 jour"
                : `dans ${getDaysLeft(activity.date)} jours`
            }}
          </p>
        </div>
      </div>
      ...
      <div class="absolute top-16 left-0 right-0 bottom-0">
        <img
          ...
          :src="'/src/assets/images/activities/' + activity.photos[0]"
          alt="Preview Image"
          class="w-full h-full object-cover cursor-pointer"
          @click="openCarousel(activity.photos)"
        />
  ...
  <div
    v-if="isCarouselOpen"
    class="fixed inset-0 z-50 bg-black bg-opacity-75 flex justify-center items-center"
  ...
  >
    ...
    >
      <SwiperSlide v-for="(photo, index) in currentPhotos" :key="index">
        <img
          :src="'/src/assets/images/activities/' + photo"
...
import activities from "/src/assets/data/activities.json";
